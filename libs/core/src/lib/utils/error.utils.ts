 ...
 * si cumple con el formato estandar del backend.
 */
 ...
 * Prioriza el formato estandar del backend, con fallback al formato legacy.
 */
export function extractErrorMessage(err: unknown, fallback: string): string {
  const httpErr = err as HttpErrorResponse | undefined;
  if (httpErr?.error) {
    // Formato estandar: { success: false, error: { code, message } }
